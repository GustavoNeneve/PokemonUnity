    ...
    public IEnumerator interact()
    {
        if (PlayerMovement.player.setCheckBusyWith(this.gameObject))
        {
            ...
            if (chosenIndex == 1)
            {
                ...
                StartCoroutine(ScreenFade.main.Fade(true, 0.4f));
                yield return new WaitForSeconds(0.2f);

                nurse.setDirection(2);

                Dialog.drawDialogBox();
                yield return StartCoroutine(Dialog.drawText("It was a good nap."));
                while (!Input.GetButtonDown("Select") && !Input.GetButtonDown("Back"))
                {
                    yield return null;
                }
                Dialog.drawDialogBox();
                yield return StartCoroutine(Dialog.drawText("You and your Pokémon are fine now."));
                while (!Input.GetButtonDown("Select") && !Input.GetButtonDown("Back"))
                {
                    yield return null;
                }
