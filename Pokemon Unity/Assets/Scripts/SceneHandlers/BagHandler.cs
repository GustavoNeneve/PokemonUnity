...
public class BagHandler : MonoBehaviour
{
    ...
    public int unselectedItemIconX = 690;
    public int unselectedItemNameX = 795;
    ...
    private void updateSelectedItem()
    {
        for (int i = 0; i < 8; i++)
        {
            if (currentPosition[currentScreen] == i)
            {
                ...
                itemIcon[i].pixelInset = new Rect(690 - 20, itemSlot[i].pixelInset.y - 4, 120, 120);
                itemIcon[i].transform.localPosition = new Vector3(0, 0, 21.5f);
                itemName[i].pixelOffset = new Vector2(unselectedItemNameX + 636, itemName[i].pixelOffset.y);
                itemNameShadow[i].pixelOffset = new Vector2(unselectedItemNameX + 641, itemNameShadow[i].pixelOffset.y);
            }
            else
            {
                if (switching)
                ...
                itemIcon[i].pixelInset = new Rect(690, itemSlot[i].pixelInset.y, 80, 80);
                itemIcon[i].transform.localPosition = new Vector3(0, 0, 20.5f);
                itemName[i].pixelOffset = new Vector2(795, itemName[i].pixelOffset.y);
                itemNameShadow[i].pixelOffset = new Vector2(795 + 1, itemNameShadow[i].pixelOffset.y);
            }
        }
    }
    ...
    private IEnumerator scrollScrollBar(int direction)
    {
        ...
        if (direction > 0)
        {
            ...
            while (increment < 1)
            {
                ...
                itemList.position = new Vector3(0, (0.415f * increment), 0);
                yield return null;
            }
        }
        else
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            else
            {
                ...
                partyHPBar[i].pixelInset = new Rect(partyHPBar[i].pixelInset.x, partyHPBar[i].pixelInset.y,
                    Mathf.FloorToInt(240f * ((float) currentPokemon.getCurrentHP() / (float) currentPokemon.getHP())),
    ...
    private IEnumerator runItemEffect(ItemData selectedItem, Pokemon currentPokemon, bool booted)
    {
        if (selectedItem.getItemEffect() == ItemData.ItemEffect.HP)
        {
                ...
                currentPokemon.getStatus() != Pokemon.Status.FAINTED)
            {
                ...
                float difference =
                    Mathf.Floor(240f * ((float) currentPokemon.getCurrentHP() / (float) currentPokemon.getHP())) -
                    startLength;
                float increment = 0;
                float speed = 0.5f;
